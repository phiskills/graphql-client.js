...
describe('Query items', () => {
  async function getItem (value: any) {
    const fn = client?.buildQuery(
      `GetItem($id: Int!) { item(id: $id) { name } }`
    )
    return fn?.(value)
  }
  ...
  test('get existing item', () =>
    expect(getItem({ id: 2 })).resolves.toEqual({ item: { name: 'Item 2' } }))
  test('get item not found', () =>
    expect(getItem({ id: 5 })).resolves.toEqual({ item: null }))
...
describe('Push items', () => {
  async function pushItem (value: any) {
    const fn = client?.buildMutation(
      `PushItem($id: Int!, $name: String!) { push(id: $id, name: $name) { id, name } }`
    )
    return fn?.(value)
  }
  const item = { id: 4, name: 'Item 4' }
  const result = [...items, item]
  test('push item succeed', () =>
    expect(pushItem(item)).resolves.toEqual({ push: result }))
  test('push item failed', () =>
    expect(pushItem({})).rejects.toEqual(new Error()))
})
